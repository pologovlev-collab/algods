...
Полезные ориентиры: чтение `vector[i]` или `list[i]` — `O(1)`; добавление в конец динамического массива — амортизированное `O(1)`; удаление из начала массива — `O(n)` из-за сдвига; поиск в неотсортированном массиве — `O(n)`.
