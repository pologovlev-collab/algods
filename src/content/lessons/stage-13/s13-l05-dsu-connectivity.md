---
{
  ...
  "learningBlocks": [
    {
      "id": "dsu-amortized-cost",
      "type": "complexity",
      "placement": "after-content",
      "title": "Две оптимизации работают вместе",
      "body": "Оценка DSU относится к последовательности операций: дорогой путь, который мы прошли сейчас, становится короче для будущих запросов.",
      "steps": [
        { "label": "Union by size", "detail": "Меньшее дерево входит в большее, поэтому глубина не растёт линейной цепочкой." },
        { "label": "Path compression", "detail": "Find перенаправляет посещённые вершины прямо к найденному представителю." },
        { "label": "Последовательность запросов", "detail": "Оплаченная работа упрощает следующие find, поэтому важна амортизированная оценка." }
      ],
      "result": "m операций над n элементами: O(m · α(n)) амортизированно."
    }
  ]
}
---
